...
Everything is SVG/SMIL so no JavaScript is required in the README. Each traveller also has a portrait base transform so GitHub loop resets cannot fall back to (0,0).
...
def portrait_points(theme: str, rng: np.random.Generator) -> np.ndarray:
    ...
    crop = image.crop((310, 42, 1218, 1218)).resize((420, 500), Image.Resampling.LANCZOS)
    ...
    # Dither at higher source resolution, then map back into the same banner viewport.
    points = np.column_stack((84 + xs * (322.0 / 420.0),
                              148 + ys * (384.0 / 500.0))).astype(np.float32)
    ...
    if len(points) > 24000:
        points = points[rng.choice(len(points), 24000, replace=False)]
    return points
...
def add_particle(parts, idx, n, frames, times, style, color, rng):
    # Keep traveller opacity CONSTANT.
    #
    # Critical loop fix:
    # The previous SVG relied on animateTransform alone, leaving every circle's
    # base transform at (0,0). GitHub's SVG renderer can briefly fall back to
    # that base state when an indefinite SMIL animation wraps from 1.0 -> 0.0.
    # That caused the FIRST portrait (initial page render) to look sharp, but
    # the portrait at the start of the SECOND loop to appear to rebuild from
    # nothing / lose definition. We now give every traveller the exact portrait
    # position as its BASE transform, so a loop reset is visually identical to
    # frame zero even before SMIL reapplies the first keyframe.
    ...
    parts.append(
        f'<circle transform="translate({num(first[0])} {num(first[1])})" r="{r}" fill="{color}" opacity="{op}">'
